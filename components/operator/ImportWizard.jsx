...
import { dryRunProductRows, commitProductRows, finalizeProductBatch, exportProductsCsv, parseSpreadsheet, downloadTemplateXlsx } from "@/lib/operator/importActions";
...
export default function ImportWizard({ recentLogs = [] }) {
  ...
  async function downloadTemplate() {
    ...
    setTplBusy(false);
    if (res?.ok) downloadBase64(res.filename || "medoria-products-template.xlsx", res.base64, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  ...
  return (
    <>
      <PageHeader title="ورود از Excel/CSV" subtitle="فایل را آپلود کنید، پیش‌نمایش را بررسی کنید، بعد تأیید کنید — قبل از تأیید چیزی ذخیره نمی‌شود.">
        ...
        <Link href="/operator/products/bulk" className="btn-ghost size-md"><Icon name="plusCircle" size={16} /> افزودن گروهی دستی</Link>
      </PageHeader>
