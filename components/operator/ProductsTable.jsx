...
import { setProductActive, duplicateProduct } from "@/lib/operator/actions";
import {
...
export default function ProductsTable({ products, categories }) {
  ...
  const rows = useMemo(() => {
    ...
    if (needle) {
      list = list.filter((p) =>
        [p.name_fa, p.name_en, p.name_ru, p.name_tg, p.sku, p.brand]
  ...
  function toggleActive(p) {
    ...
    startTransition(async () => {
      ...
      setBusyId(null);
      if (res.ok) { flash(p.is_active === false ? "محصول فعال شد." : "محصول آرشیو شد.", true); router.refresh(); }
      ...
      {rows.length === 0 ? (
        ...
        <div className="card divide-y divide-line overflow-hidden">
          {rows.map((p) => {
            ...
            return (
              <div key={p.id} className="px-3 sm:px-4 py-3 flex items-center gap-3 hover:bg-line-soft/50 transition-colors">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-ink-muted truncate mt-0.5">
                    <span className="tabular">{p.sku || "—"}</span> · {nameOf(p)} · <span className={isOnRequest(p) ? "text-brand-violet" : "text-ink-soft"}>{priceText(p)}</span>
                  </p>
                </div>
                <Badge tone={active ? "ok" : "muted"} className="hidden sm:inline-flex">{active ? "فعال" : "غیرفعال"}</Badge>
                {!p.in_stock && <Badge tone="warn" className="hidden md:inline-flex">ناموجود</Badge>}
