"use client";
// components/beauty/operator/QuickCategoryModal.jsx — minimal "new category" dialog
// usable inline from the product form / bulk grid without leaving the page.
// Full editing (icon, image, descriptions) stays in /beauty/operator/categories.
import { useState } from "react";
import Icon from "@/components/shared/Icon";
import { createCategory } from "@/lib/beauty/operator/actions";
import { slugify } from "@/lib/operator/validation";
import { BEAUTY_WORLDS } from "@/lib/beauty/operator/constants";
import { Field, Input, Select, Spinner } from "@/components/operator/ui";

export default function QuickCategoryModal({ onClose, onCreated }) {
  const [f, setF] = useState({ name_fa: "", name_en: "", slug: "", world: "" });
    ...
    <div className="fixed inset-0 z-50 grid place-items-center p-4" onClick={onClose}>
      ...
      <div onClick={(e) => e.stopPropagation()} dir="rtl" className="relative card p-6 w-full max-w-md animate-fade-in">
        ...
        <div className="grid gap-4">
          <Field label="نام (فارسی)" required><Input value={f.name_fa} onChange={(e) => up("name_fa", e.target.value)} placeholder="رژ لب" /></Field>
          <Field label="نام (انگلیسی)"><Input value={f.name_en} onChange={(e) => { up("name_en", e.target.value); if (!slugTouched) up("slug", slugify(e.target.value)); }} dir="ltr" placeholder="Lip Makeup" /></Field>
          <Field label="اسلاگ" required hint="حروف لاتین — در آدرس صفحه دسته">
            <Input value={f.slug} onChange={(e) => { setSlugTouched(true); up("slug", e.target.value); }} dir="ltr" placeholder="lip-makeup" />
          </Field>
          <Field label="دنیا" hint="دسته زیر کدام دنیای Beauty نمایش داده شود">
