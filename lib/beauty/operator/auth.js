...
// ── Password (scrypt) ─────────────────────────────────────────────────────────
// New format:     scrypt:<N>:<r>:<p>:<saltHex>:<keyHex>  (scripts/hash-operator-password.mjs)
// Legacy format:  scrypt:<saltHex>:<keyHex>  (Node defaults, N=2^14) — still
//                 verified so an existing BEAUTY_OPERATOR_PASSWORD_HASH keeps
//                 working. Cost follows OWASP 2024/2025 (N=2^17); maxmem raised.
...
export async function getSession() {
  ...
  const token = cookieStore.get(BOP_COOKIE)?.value;
  const payload = await verifySession(token);
  if (!payload) return null;
  if (payload.role !== "beauty-admin") return null;     // reject a token minted for another panel (defense-in-depth)
