...
// Every submission must pass, in order: honeypot → server-validated math
// captcha → per-IP rate limits → field validation/caps. Next.js server
// actions already enforce a same-origin check on the request itself.
//
// Storage: PRODUCTION requires the service-role key — there is intentionally
// NO anon-key fallback once deployed, because migration 09 (step 3) drops the
// anon insert policy on quote_requests; a silent fallback would just produce
// confusing RLS errors. Development may still fall back to the anon key.
...
function getWriteClient() {
  ...
  if (!service && process.env.NODE_ENV === "production") {
    // Strict by design — see header comment. Generic message reaches the user.
    console.error("submitQuoteRequest: SUPABASE_SERVICE_ROLE_KEY is not set in production — refusing anon-key fallback for protected writes.");
    return null;
  }
...
//            language, productId, productName, productSku,
...
export async function submitQuoteRequest(payload = {}) {
  try {
    ...
    const [m, h] = await Promise.all([
      rateLimit(`quote:m:${ip}`, { limit: 5, windowMs: 60_000 }),
      rateLimit(`quote:h:${ip}`, { limit: 20, windowMs: 3_600_000 }),
    ]);
