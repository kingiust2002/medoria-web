...
export function gatewayLumenMedia() {
  ...
  // Optional portrait crops for the mobile two-door split. If absent, the
  // mobile layout center-crops the landscape stills above.
  const moCool = find(dir, ["mobile-cold", "mobile-cool"], IMG_EXTS);
  const moWarm = find(dir, ["mobile-warm"], IMG_EXTS);
  return {
    scene: cool && warm ? { cool, warm } : null,
    mobile: moCool && moWarm ? { cool: moCool, warm: moWarm } : null,
    video: {
