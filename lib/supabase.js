...
// Newsletter signup → newsletter_subscribers (anon insert). Low-maintenance:
// just collects emails; a duplicate (unique violation) is treated as success.
export async function subscribeNewsletter(email, lang = "en") {
  const clean = String(email || "").trim().toLowerCase();
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(clean)) return { ok: false, error: "invalid" };
  const { error } = await supabase
    .from("newsletter_subscribers")
    .insert({ email: clean, language: lang });
  if (error) {
