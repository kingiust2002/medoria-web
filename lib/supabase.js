...
export async function getProducts(opts = {}) {
  ...
  if (categorySlug) {                       // convenience: resolve slug → id
    const cat = await getCategoryBySlug(categorySlug);
    if (cat?.id != null) q = q.eq("category_id", cat.id);
    else q = q.eq("category", categorySlug); // pre-migration fallback
  }
...
export async function getRelatedProducts(product, n = 4) {
  ...
  const { data, error } = await supabase
    ...
    .eq(col, val).neq("id", product.id)
    .order("is_featured", { ascending: false }).limit(n);
