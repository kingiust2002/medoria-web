...
function buildProductRow(input) {
  ...
  return {
    ...
    image_url: str(input.image_url, 600),
    gallery_urls: strArray(input.gallery_urls, { maxItems: 12, maxLen: 600 }),
    specs: parseSpecs(input.specs),
    brochure_url: str(input.brochure_url, 600),
  ...
  try {
    for (const target of targets) {
      ...
      if (!resp.ok || !json?.data?.translations) {
        ...
        return { ok: false, error: msg ? `خطای سرویس ترجمه: ${msg}` : "ترجمه ناموفق بود." };
      }
      translations[target] = {};
