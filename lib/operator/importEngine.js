// lib/operator/importEngine.js — SERVER ONLY
// Executes product batches (bulk add + Excel/CSV import) against Supabase.
// All DB state is loaded once per call; planning itself is pure (importCore).
// Never imported by client code — actions in importActions.js wrap it.
import "server-only";
...
import { fillTranslationsForWrites } from "@/lib/operator/importTranslate";

// Friendly error text for per-row write failures (mirrors actions.js).
function friendlyRowError(error) {
  const msg = String(error?.message || error || "");
  if (/products_sku_unique_idx/i.test(msg) || (error?.code === "23505" && /sku/i.test(msg)))
    return "این کد محصول (SKU) همین حالا توسط ردیف/محصول دیگری ثبت شد.";
  if (/products_slug_unique_idx/i.test(msg) || (error?.code === "23505" && /slug/i.test(msg)))
  ...
  if (error?.code === "42703" || error?.code === "PGRST204" || /column .* does not exist/i.test(msg))
    return "ستون موردنیاز وجود ندارد — مایگریشن‌های ۰۴ تا ۰۷ را اجرا کنید.";
...
// One catalog-sized read per call — far cheaper and simpler than per-row
// lookups, and correct for case-insensitive SKU matching.
async function loadContext(admin) {
  const [prodRes, catRes] = await Promise.all([
    admin.from("products").select("id,sku,slug"),
    admin.from("categories").select("id,slug,is_active"),
  ]);
...
// Returns { ok, results, counts, createdCategories } or { ok:false, error }.
export async function processProductBatch(rawRows, options = {}) {
  ...
  try {
    ...
    return { ok: false, error: "خواندن داده‌های موجود از پایگاه داده ناموفق بود." };
  }
  ...
  for (const cat of categoriesToCreate) {
    ...
    if (error) {
      ...
      const { data: ex } = await admin.from("categories").select("id,slug").eq("slug", cat.slug).single();
  ...
  // 1b) auto-translate empty name/description languages on NEW products, from
  //     whichever language the operator filled. Best-effort; never blocks.
  if (options.autoTranslate) {
    const createWrites = plans.filter((p) => p.action === "create").map((p) => p.write);
    await fillTranslationsForWrites(createWrites, process.env.GOOGLE_TRANSLATE_API_KEY);
  }
  ...
  for (const plan of plans) {
    ...
    if (plan.action === "create") {
      const { data, error } = await admin.from("products").insert(write).select("id,slug").single();
      ...
      const { error } = await admin.from("products").update(write).eq("id", plan.id);
      if (error) results.push({ ...base, status: "error", messages: [...base.messages, friendlyRowError(error)] });
...
// Best-effort import history (table from migration 08). Missing table → no-op.
