...
// ── Password (scrypt) ─────────────────────────────────────────────────────────
// New format:     scrypt:<N>:<r>:<p>:<saltHex>:<keyHex>
// Legacy format:  scrypt:<saltHex>:<keyHex>  (Node defaults, N=2^14) — still
//                 verified so a previously-generated OPERATOR_PASSWORD_HASH
//                 keeps working; only newly generated hashes use the stronger N.
// Cost follows OWASP 2024/2025 guidance (N=2^17). maxmem must be raised or Node
// rejects an N this large (needs ~128*N*r ≈ 134 MB).
const SCRYPT = { N: 131072, r: 8, p: 1, maxmem: 256 * 1024 * 1024 };

export function hashPassword(password) {
  const salt = randomBytes(16);
  const key = scryptSync(String(password), salt, 32, SCRYPT);
...
export function verifyPassword(password) {
  ...
  if (parts.length === 3) {
    [, saltHex, keyHex] = parts;                 // legacy: Node defaults
...
export async function getSession() {
  ...
  const token = cookieStore.get(OP_COOKIE)?.value;
  const payload = await verifySession(token);
  if (!payload) return null;
  if (payload.role !== "admin") return null;            // reject a token minted for another panel (defense-in-depth)
