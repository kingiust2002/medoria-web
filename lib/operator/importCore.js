...
import { slugify, str, parseSpecs, safeUrlRef } from "@/lib/operator/validation";
...
export const MAX_NEW_CATEGORIES = 50;   // per-import cap on auto-created categories
...
//   "auto"     (green) — leave empty and the importer fills it (SKU/slug/flags,
//                        and the other languages via auto-translation)
//   "optional" (blue)  — fill if you want it, otherwise ignored
...
// Derive a stable SKU from the (already unique) slug when the operator leaves
// the SKU blank on a new product — so every imported row is later updatable by
// SKU. Slugs are unique per catalog, so the derived SKU is unique too.
export function autoSkuFromSlug(slug) {
...
// Accepts an http(s) URL or a storage path like "products/abc.webp" — and
// nothing else (no javascript:/data:/protocol-relative). Single source of truth
// with the manual-form guard so both entry paths reject the same values.
export function isImageRef(s) {
  return safeUrlRef(s) !== null;
}
...
function buildCreateRow(fields, { category, category_id, slug }) {
  return {
    ...
    slug,
    // Blank SKU on a new product → derive a stable one from the unique slug.
    sku: fields.sku || autoSkuFromSlug(slug),
    brand: fields.brand,
