...
export default function CatalogPage({ params }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (sortIdx === 1) res = [...res].sort((a, b) => a.price - b.price);
    if (sortIdx === 2) res = [...res].sort((a, b) => b.price - a.price);
    if (sortIdx === 3) res = [...res].sort((a, b) => (a[`name_${lang}`] || "").localeCompare(b[`name_${lang}`] || ""));
    return res;
  }, [products, search, cat, brand, badge, sortIdx, stockOnly, lang]);
      ...
      <div className="container-x py-8 md:py-10">
        ...
        <div className="flex flex-col md:flex-row gap-3 mb-5">
          <div className="relative flex-1">
            <Icon name="search" size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-faint pointer-events-none" />
            <input
              value={search}
              onChange={(e) => { setSearch(e.target.value); reset(); }}
              placeholder={t.catalog.searchPh}
              className="input w-full h-12 pl-12 pr-10 rounded-2xl shadow-soft"
            />
            {search && (
          ...
          >
            ...
            <option value={1}>{t.catalog.sortPriceAsc}</option>
            <option value={2}>{t.catalog.sortPriceDesc}</option>
            <option value={3}>{t.catalog.sortName}</option>
          </select>
        </div>
        ...
        <div className="flex gap-2 mb-6 flex-wrap items-center">
          <span className="text-[11px] text-ink-faint uppercase tracking-wide mr-1">{t.catalog.filterBadge}:</span>
          ...
          <>
            <div className={view === "grid"
              ...
              {filtered.slice(0, visible).map((p) => (
                <ProductCard key={p.id} product={p} lang={lang} view={view} />
              ))}
            </div>
