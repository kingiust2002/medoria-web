    ...
    <div className="bg-canvas-soft">
      <section className="bg-canvas-soft border-b border-line relative overflow-hidden">
        ...
        <div className="container-x py-12 md:py-20 relative">
          <Breadcrumb lang={lang} className="mb-4" crumbs={[{ label: t.nav.home, href: `/beauty/${lang}` }, { label: t.nav.contact }]} />
      ...
      <div className="container-x py-12 md:py-16">
        <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
          <div className="space-y-3 lg:col-span-1">
            <MethodCard icon="chat" iconBg="bg-ok/10 text-ok" label={c.labels.whatsapp} value={`+${WA_NUMBER}`} desc={c.methods.whatsapp} href={waLink("")} />
            <MethodCard icon="send" iconBg="bg-[#229ED9]/10 text-[#229ED9]" label={c.labels.telegram} value={`@${TG_USER}`} desc={c.methods.telegram} href={`https://t.me/${TG_USER}`} />
            <MethodCard icon="mail" iconBg="bg-brand-violet/10 text-brand-violet" label={c.labels.email} value={email} desc={c.methods.email} href={`mailto:${email}`} />
            <MethodCard icon="mapPin" iconBg="bg-[color:var(--v-copper)]/10 text-[color:var(--v-copper)]" label={c.labels.address} value={c.methods.address} desc={c.methods.address} />

            <div className="card p-5">
              <h3 className="font-semibold text-ink text-[14px] mb-3 flex items-center gap-2">
                <Icon name="clock" size={16} className="text-brand-violet" />
                {c.hours.title}
              </h3>
...
function MethodCard({ icon, iconBg, label, value, desc, href }) {
  const inner = (
    <>
  ...
  return href ? (
    <TiltCard className="rounded-2xl" max={6}>
      <a href={href} target="_blank" rel="noopener noreferrer" className="card p-5 flex items-start gap-4 card-hover transition-all">{inner}</a>
    </TiltCard>
  ) : (
    <TiltCard className="rounded-2xl" max={6}>
...
const CHANNELS = [["whatsapp", "chat"], ["telegram", "send"], ["phone", "phone"]];
const CHANNEL_ACTIVE = {
  ...
  phone: "bg-[color:var(--v-navy)] text-white border-[color:var(--v-navy)] shadow-[0_6px_16px_rgba(28,41,81,0.30)]",
};

function ContactForm({ lang, c }) {
  const [form, setForm] = useState({ name: "", org: "", phone: "", email: "", subject: "", message: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    const msg = [
      lang === "fa" ? "📝 پیام از مدوریا بیوتی" : lang === "tg" ? "📝 Паём аз Medoria Beauty" : lang === "en" ? "📝 Medoria Beauty message" : "📝 Сообщение Medoria Beauty",
      "",
      `👤 ${c.form.name}: ${form.name || "—"}`,
      `🏢 ${c.form.org}: ${form.org || "—"}`,
      `📞 ${c.form.phone}: ${form.phone || "—"}`,
      form.email ? `✉ ${c.form.email}: ${form.email}` : "",
    ...
    if (via === "phone") window.location.href = `tel:${PHONE}`;
    else window.open(via === "telegram" ? tgLink(msg) : waLink(msg), "_blank", "noopener");
    setSending(false);
    setSent(true);
  };
  ...
  if (sent) {
    return (
      <div className="card p-8 md:p-12 text-center">
        ...
        <button onClick={() => { setSent(false); setForm({ name: "", org: "", phone: "", email: "", subject: "", message: "" }); }} className="btn-contact size-md">
          {c.form.another}
        </button>
      </div>
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="card p-6 md:p-8">
      ...
      <input type="text" value={hp} onChange={(e) => setHp(e.target.value)} name="website" tabIndex={-1} autoComplete="off" aria-hidden="true" className="absolute opacity-0 h-0 w-0 pointer-events-none" />

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        <Field label={c.form.name} required><input value={form.name} onChange={update("name")} required maxLength={150} className="input w-full" /></Field>
        <Field label={c.form.org}><input value={form.org} onChange={update("org")} maxLength={160} className="input w-full" /></Field>
        <Field label={c.form.phone} required><input value={form.phone} onChange={update("phone")} type="tel" required maxLength={40} className="input w-full" /></Field>
        <Field label={c.form.email}><input value={form.email} onChange={update("email")} type="email" maxLength={120} className="input w-full" /></Field>
      </div>

      <Field label={c.form.subject} className="mb-4"><input value={form.subject} onChange={update("subject")} maxLength={150} className="input w-full" /></Field>
      <Field label={c.form.message} required className="mb-5">
        <textarea value={form.message} onChange={update("message")} required rows={5} maxLength={2000} className="input w-full h-auto py-3 resize-none" />
      </Field>
        ...
        <div className="grid grid-cols-3 gap-2">
          {CHANNELS.map(([v, icon]) => (
            <button key={v} type="button" onClick={() => setVia(v)}
              className={["h-11 rounded-xl text-[13px] font-semibold border transition-colors flex items-center justify-center gap-2",
                via === v ? CHANNEL_ACTIVE[v] : "bg-surface text-ink-muted border-line hover:border-ink-faint"].join(" ")}>
              <Icon name={icon} size={15} />
              {v === "phone" ? c.form.viaPhone : c.labels[v]}
            </button>
          ))}
        </div>
      </div>

      {formError ? <p role="alert" className="text-[11px] text-warn mb-3 text-center">{formError}</p> : null}
      <button type="submit" disabled={sending} className="btn-primary size-xl w-full disabled:opacity-60">
        ...
        {via === "phone" ? c.form.viaPhone : c.form.send}
      </button>
    </form>
  );
}
...
function Field({ label, required, children, className = "" }) {
  return (
    <div className={className}>
      <label className="block text-[11px] font-semibold text-ink mb-1.5">{label} {required && <span className="text-warn">*</span>}</label>
      {children}
    </div>
  );
}
