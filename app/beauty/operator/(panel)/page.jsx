// app/beauty/operator/(panel)/page.jsx — Beauty dashboard.
import Link from "next/link";
import Icon from "@/components/shared/Icon";
import { beautyImageUrl as imageUrl } from "@/lib/beauty/catalog";
import { getBeautyDashboardData } from "@/lib/beauty/operator/data";
import { isAdminConfigured } from "@/lib/operator/supabaseAdmin";
import { QUOTE_STATUS_FA, QUOTE_STATUS_TONE } from "@/lib/beauty/operator/constants";
import {
...
const QUICK = [
  { href: `${BASE}/products/new`, label: "افزودن محصول", icon: "plus", primary: true },
  { href: `${BASE}/products`, label: "مدیریت محصولات", icon: "package" },
  { href: `${BASE}/categories`, label: "دسته‌بندی‌ها", icon: "layers" },
  { href: `${BASE}/quotes`, label: "استعلام‌ها", icon: "quote" },
];

export default async function BeautyDashboardPage() {
  const configured = isAdminConfigured();
  const { stats, recentProducts, recentQuotes } = await getBeautyDashboardData();

  return (
    <>
      <PageHeader title="داشبورد بیوتی" subtitle="نمای کلی کاتالوگ لوکس و استعلام‌ها">
        <Link href={`${BASE}/products/new`} className="btn-primary size-md">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <SectionCard title="استعلام‌های اخیر" icon="quote">
          ...
          <Link href={`${BASE}/quotes`} className="inline-flex items-center gap-1.5 text-xs font-semibold text-brand-violet mt-4 hover:gap-2.5 transition-all">
        ...
        <SectionCard title="محصولات اخیراً به‌روزشده" icon="refresh">
          {recentProducts?.length ? (
                ...
                return (
                  <li key={p.id} className="py-3 flex items-center gap-3 first:pt-0 last:pb-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-ink-muted truncate">{[p.brand, p.sku].filter(Boolean).join(" · ") || "—"} · {priceText(p)}</p>
                    </div>
                    <Link href={`${BASE}/products/${p.id}/edit`} className="btn-ghost size-xs shrink-0">
            ...
            <EmptyState icon="sparkles" title="هنوز محصولی نیست" desc="اولین محصول لوکس کاتالوگ را اضافه کنید.">
              <Link href={`${BASE}/products/new`} className="btn-primary size-sm"><Icon name="plus" size={16} /> افزودن محصول</Link>
            </EmptyState>
          )}
        </SectionCard>
      </div>
