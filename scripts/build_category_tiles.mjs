...
if (!existsSync(SRC_DIR)) {
  console.error(`No source folder yet: assets/category-source/`);
  console.error(`Create one folder per group slug and drop 3-5 official product shots in each, e.g.`);
  console.error(`  assets/category-source/face-care/{serum,cream,pump,ampoule,tube}.png`);
  process.exit(1);
}

mkdirSync(OUT_DIR, { recursive: true });

const groups = readdirSync(SRC_DIR).filter((d) => statSync(join(SRC_DIR, d)).isDirectory()).sort();
if (!groups.length) {
  console.error(`assets/category-source/ exists but has no group folders.`);
  process.exit(1);
}
...
// Generated manifest — WorldGrid consults this only when a category has no
// uploaded image_url of its own, so a panel upload always wins.
...
${done.map((s) => `  ${JSON.stringify(s)}: "/beauty/categories/${s}.webp",`).join("\n")}
};
`;
writeFileSync(MANIFEST, body);
console.log(`\nwrote lib/beauty/categoryImages.js — ${done.length} tiles`);
