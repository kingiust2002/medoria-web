...
test("passes when NEXT_PUBLIC_SITE_URL matches STAGING_HOST (temporary staging server)", async () => {
  const { code, stdout } = await runChecker({
    STAGING_HOST: "staging-preview.medoria-test.dev",
    NEXT_PUBLIC_SITE_URL: "https://staging-preview.medoria-test.dev",
  });
  assert.equal(code, 0);
  assert.match(stdout, /preflight passed/);
});

test("passes when NEXT_PUBLIC_SITE_URL is a host deploy/Caddyfile already routes, even if it differs from STAGING_HOST", async () => {
  const { code, stdout } = await runChecker({
    STAGING_HOST: "staging.medoriaco.com",
...
test("fails when NEXT_PUBLIC_SITE_URL is neither STAGING_HOST nor a Caddy-routed production host", async () => {
  const { code, stderr } = await runChecker({
    STAGING_HOST: "staging.medoriaco.com",
    NEXT_PUBLIC_SITE_URL: "https://typo-domain.example.com",
  });
  assert.equal(code, 1);
  assert.match(stderr, /NEXT_PUBLIC_SITE_URL hostname must equal STAGING_HOST/);
});
