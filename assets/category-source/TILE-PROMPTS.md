...
as **one campaign**: same marble ledge, same silk backdrop, same champagne
light from the left, same palette. That consistency comes from reusing one
fixed shell and changing only the SUBJECT line. Regenerating a single tile
months from now has to produce something that still sits in the same set —
hence the shell below is verbatim, not a summary.
...
Everything below is sent verbatim, with `{SUBJECT}` replaced:
...
> SUBJECT: {SUBJECT}
>
> SETTING AND LIGHT: the items stand in a row on a pale cream marble ledge that
> runs across the lower third of the frame. Behind them, soft ivory silk fabric
> is draped in gentle folds, slightly out of focus. Warm champagne studio light
> rakes in from the left, casting soft shadows to the right across the marble.
> The palette is strictly ivory, cream, warm beige, champagne gold and polished
> copper — no cool blues, no colored gels, no strong saturation.
>
> CAMERA: shot straight on at product height with an 85mm lens, shallow depth of
> field so the silk background is softly blurred while the products stay crisp.
> Editorial luxury beauty campaign photography, high-end retail merchandising
> quality, calm and expensive. Landscape 16:9 composition, the row centred with
> generous empty space above.

## SUBJECT per group

Status: ✅ = tile committed · ⬜ = still to generate.
